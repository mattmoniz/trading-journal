...
- `scripts/backtest_confluence_globex.js` (2026-07-22, manual-only, no cron entry yet) — Globex/overnight counterpart to `backtest_confluence.js`, built per user directive that any new setup/calibration finding must be tested against BOTH RTH and Globex windows before being considered complete. Mirrors the RTH script's touch-detection/dedup/pairing/rigor pipeline (reuses its exported constants/helpers/`loadPairProximityThresholds()` directly rather than duplicating — level spacing is a property of the levels, not the session window) but scans the overnight window (6PM–8:30AM ET, spanning a calendar-date boundary) instead of RTH hours, and looks up level values as of the PRIOR trading day (not the session's own date) for lookahead safety. Same-day-forming levels (`OR_HIGH`/`OR_LOW`/`OR_MID`/`IB_HIGH`/`IB_LOW`/`IB_MID`), circularly-defined ones (`ONH`/`ONL`), and `RTH_VWAP` (live-accumulates through its own date) are excluded from the candidate set entirely rather than reasoned through case-by-case under time pressure. Writes `CONFLUENCE_AUDIT_OVERNIGHT` rows (distinctly named, kept separate from RTH's `CONFLUENCE_AUDIT`, matching the established `_OVERNIGHT`-suffix convention). Real result: 421 overnight sessions, 890 qualified (N≥10) pairs, only **5 clear the full validation bar** (much thinner than RTH's 71/1461 — overnight structure is less consistent) — `PD_IB_LOW+PD_LOW` ($10.56, N=1354), `FLOOR_PIVOT+PD_SESSION_MID` ($7.01, N=1422), `PD_HIGH+PD_IB_HIGH` ($6.83, N=2225), `PD_CLOSE+PD_POC` ($4.31, N=1254), `CAM_S4+FLOOR_S1` ($4.06, N=998) — 2 of these also independently validate on the RTH side, genuinely cross-session-robust. `confluence_score_at_detection`/`confluence_levels_at_detection` are now persisted on every `detectGlobexSetup()` insert (same fields as RTH), but **no sizing-bonus mechanism exists to wire the validated pairs into** — `detectGlobexSetup()` has no `sizeMultiplier` stack at all, unlike the RTH engine — flagged as `OPEN_DECISION` `globex_confluence_pair_bonus_needs_sizing_mechanism` rather than built ad hoc. See `confluence_pair_globex_backtest` `RESEARCH_CLAIM` for the full write-up, including a real bug found mid-build (an unguarded `run()` call in the RTH script raced a concurrent execution the first time this file imported from it — fixed with the standard entry-point guard).
