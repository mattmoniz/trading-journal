    ...
    <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: 8, marginBottom: 12, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, padding: '8px 14px', borderBottom: '1px solid #1e293b' }}>
        <div style={{ padding: '6px 8px', background: 'rgba(30,41,59,0.3)', borderRadius: 4, borderLeft: `3px solid ${Math.abs(L.dailyVwapSigma || 0) >= 2 ? '#fbbf24' : Math.abs(L.dailyVwapSigma || 0) >= 1 ? '#fb923c' : '#64748b'}` }}>
          <div style={{ fontSize: 9, color: '#64748b', fontWeight: 600, textTransform: 'uppercase' }}>RTH VWAP</div>
          ...
          <div style={{ fontSize: 9, color: '#64748b' }}>{fmtP(L.vwap)} ({L.vwapDist > 0 ? '+' : ''}{L.vwapDist}pt)</div>
        </div>
        <div style={{ padding: '6px 8px', background: 'rgba(30,41,59,0.3)', borderRadius: 4, borderLeft: `3px solid ${Math.abs(L.vwap24Sigma || 0) >= 2 ? '#fbbf24' : Math.abs(L.vwap24Sigma || 0) >= 1 ? '#fb923c' : '#64748b'}` }}>
          <div style={{ fontSize: 9, color: '#64748b', fontWeight: 600, textTransform: 'uppercase' }}>24HR VWAP</div>
          ...
          <div style={{ fontSize: 9, color: '#64748b' }}>{L.vwap24 ? `${fmtP(L.vwap24)} (${L.vwap24Dist > 0 ? '+' : ''}${L.vwap24Dist}pt)` : '—'}</div>
        </div>
        <div style={{ padding: '6px 8px', background: 'rgba(30,41,59,0.3)', borderRadius: 4, borderLeft: `3px solid ${Math.abs(L.weeklyVwapSigma || 0) >= 2 ? '#ef4444' : Math.abs(L.weeklyVwapSigma || 0) >= 1 ? '#fb923c' : '#64748b'}` }}>
          ...
          <div style={{ fontSize: 9, color: '#64748b' }}>{L.weeklyVwap ? `${fmtP(L.weeklyVwap)} (${L.weeklyVwapSigma != null ? Math.round(L.weeklyVwapSigma * 251) + 'pt' : '—'})` : '—'}</div>
        </div>
      </div>
