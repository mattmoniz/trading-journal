...
**This is a general pattern risk, not a one-off bug in one script** — any code that queries `price_bars_primary` with `ts >= X` and no upper bound will hit the same wall, and this codebase has many scripts doing exactly that (a `grep -rn "FROM price_bars_primary" scripts/ server/` turns up dozens). **Not yet audited across the codebase** — flagged as `OPEN_DECISION` `price_bars_primary_unbounded_query_audit` rather than fixed reflexively everywhere, since each caller needs its own reasonable upper bound (a live 30-bar lookback needs a different bound than a 2-year historical scan with its own explicit date range, which is usually already bounded and unaffected). When writing or auditing any new script that queries `price_bars_primary` with a lower-bound-only `ts` filter, add an explicit upper bound sized to what the query actually needs — don't rely on `LIMIT` alone to keep it fast, since `LIMIT` only helps after the expensive aggregation, not before it.
