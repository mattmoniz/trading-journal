...
export default function SessionForecastPanel({ date }) {
  ...
  const keyLevels = [];
  if (cl.pd1?.vah) keyLevels.push({ price: cl.pd1.vah, name: 'PD-1 VAH', behavior: 'Resistance. 4-5 retests, 5-bar dwell. Watch for absorption.', color: '#fb923c' });
  if (cl.pd1?.val) keyLevels.push({ price: cl.pd1.val, name: 'PD-1 VAL', behavior: 'Support. 2 retests, 2-bar dwell. Holds or breaks fast.', color: '#4ade80' });
  if (cl.pd1?.poc) keyLevels.push({ price: cl.pd1.poc, name: 'PD-1 POC', behavior: 'Magnet. Expect touch within 51 min. Arrives fast (16pt/bar), dwells 1 bar, departs slow.', color: '#a78bfa' });
  if (cl.pd1?.high) keyLevels.push({ price: cl.pd1.high, name: 'PD-1 High', behavior: 'Resistance. First test is the key read.', color: '#f87171' });
  if (cl.pd1?.low) keyLevels.push({ price: cl.pd1.low, name: 'PD-1 Low', behavior: 'Support. Sweep below = potential failed auction long.', color: '#4ade80' });
    ...
    <div style={{ fontSize: 12, color: '#cbd5e1' }}>
      ...
      <div style={{ padding: '10px 12px', background: 'rgba(99,102,241,0.04)', border: '1px solid rgba(99,102,241,0.15)', borderRadius: 6 }}>
        ...
        <div style={{ fontSize: 12, lineHeight: 1.7, color: '#cbd5e1' }}>
          ...
          {cl.pd1?.vah && cl.pd1?.val && <div>3. <strong style={{ color: '#818cf8' }}>VA edge test.</strong> If price reaches VAH (<strong>{fmtP(cl.pd1.vah)}</strong>): expect 4-5 retests over ~5 bars. Volume should increase 1.37x at the edge — that confirms it's a real test. If tempo slows and volume builds = fade back inside. If volume stays heavy with no dwell = breakout. VAL (<strong>{fmtP(cl.pd1.val)}</strong>) resolves faster — 2 retests, 2-bar dwell. Holds or breaks decisively.</div>}
          {playbook && <div>4. <strong style={{ color: '#818cf8' }}>{playbook.name} setup edge.</strong> {playbook.setups[0]?.name} has the best relative odds today ({playbook.setups[0]?.wr}% WR, {playbook.setups[0]?.delta} vs avg) — still requires your read at the level to confirm.{playbook.setups.find(s => s.verdict === 'AVOID') ? ` ${playbook.setups.find(s => s.verdict === 'AVOID').name} underperforms today — skip unless overnight strongly confirms.` : ''}</div>}
          ...
          <div>6. <strong style={{ color: '#818cf8' }}>10:30 AM — IB close.</strong> The Initial Balance is set. IB range defines the session framework. If IB is tight (under 47pt) → breakout expansion likely, watch for IB_BEARISH or A signal continuation. If IB is wide (over 91pt) → range day, fade the IB extremes, TRT setups become relevant.</div>
          <div>7. <strong style={{ color: '#818cf8' }}>10:30 AM — 12:00 PM: the resolution window.</strong> This is when the morning thesis either confirms or fails. If the A signal held and IB broke in the same direction → trend is developing, stay with it. If the A signal failed or IB broke the wrong way → day type is shifting, reassess. Most setups that are going to fire today fire by noon.</div>
        </div>
      </div>
      ...
      <div style={{ padding: '10px 12px', background: 'rgba(245,158,11,0.04)', border: '1px solid rgba(245,158,11,0.12)', borderRadius: 6, marginTop: 8 }}>
        ...
        <div style={{ fontSize: 12, lineHeight: 1.7, color: '#cbd5e1' }}>
          ...
          <div style={{ paddingLeft: 16, fontSize: 11, lineHeight: 1.6, color: '#94a3b8' }}>
            {cl.pd1?.vah && <div>• Close above <strong style={{ color: '#e2e8f0' }}>{fmtP(cl.pd1.vah)}</strong> (PD-1 VAH) → tomorrow opens ABOVE VALUE. Bullish setups get 61% aligned WR.</div>}
            {cl.pd1?.val && <div>• Close below <strong style={{ color: '#e2e8f0' }}>{fmtP(cl.pd1.val)}</strong> (PD-1 VAL) → tomorrow opens BELOW VALUE. IB_BEARISH at 88% WR. {dow === 1 ? 'Tuesday TRT_LONG at 83% if reversal fires.' : ''}</div>}
          ...
          <div style={{ paddingLeft: 16, fontSize: 11, lineHeight: 1.6, color: '#94a3b8' }}>
            <div>• Close well below today's open → <strong style={{ color: '#ef4444' }}>LONG_TRAPPED</strong> tomorrow. Buyers from today under pressure. Selling fuel for tomorrow's session.</div>
            <div>• Close near today's open → <strong style={{ color: '#94a3b8' }}>NEUTRAL</strong> tomorrow. No trapped participants. Standard playbook.</div>
            <div>• Close well above today's open → <strong style={{ color: '#22c55e' }}>SHORT_TRAPPED</strong> tomorrow. Sellers squeezed. Buying fuel.</div>
          </div>
          {dow === 5 && <div>5. <strong style={{ color: '#ef4444' }}>Friday PM: lock gains by 12:30.</strong> Afternoon squaring creates reversals that eat Friday profits. Be flat or trailing only.</div>}
          {dow !== 5 && <div>5. <strong style={{ color: '#f59e0b' }}>3:00-4:00 PM closing auction.</strong> MOC orders create directional flow. If price is below today's POC, closing sellers may push lower. If above, closing buyers may push higher. This is the final read for tomorrow's setup.</div>}
        </div>
      </div>
