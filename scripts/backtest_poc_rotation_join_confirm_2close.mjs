// Confirmation-gated entry (not blind delay): wait 1 or 2 minutes after convergence,
// only take the trade if the reference level L (the 24hr median at convergence,
// event.L -- same level used as the stop reference elsewhere in this thread) actually
// HOLDS during that wait (no bar closes against the trade direction through it). Skip
// the trade entirely if it breaks during the wait. User's own question, 2026-08-24:
// does this fail less, and if so can we afford farther targets? Tests both halves:
// Section 1 compares WR/EV of the CONFIRMED population against the baseline (no wait)
// under the SAME plain stop/Time60 structure (isolates "does it fail less"). Section 2
// derives wider targets from the confirmed population's own MFE distribution (mfe50/
// mfe75, data-derived, no static thresholds) to test "can we afford farther targets."
//
...
async function runConstruction(label, R, rMode, sessions, csvChunks) {
    ...
    for (const waitBars of WAIT_MINUTES) {
        const gated = waitBars === 0
            ? all_signal
            : all_signal.filter(e => {
                const session = sessions.find(s => s.t === e.t);
                const long = e.direction === 'UP';
                return levelHeldDuringWait(e, session.bars, waitBars, long);
            });
        console.log(`  --- wait=${waitBars}min: gated=${gated.length}/${all_signal.length} (${(gated.length / all_signal.length * 100).toFixed(1)}%) ---`);
...
async function main() {
    ...
    for (const result of [fixedResult, pctResult]) {
        ...
        const claimText = `Confirmation-gated entry (skip if the reference level L breaks during a 1-2min wait, not a blind delay) on the JOIN/Stop20/Time60 winner. ${result.label} construction.
${lines}`;
