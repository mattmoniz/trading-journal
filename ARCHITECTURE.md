...
| MGI levels | `level_prices` | 42 computed market-generated-info levels per session date (PD VA, overnight, OR, VWAP, floor pivots, weekly/monthly/quarterly VA). PK: `(trade_date, level_name)`. Backfilled Nov 2023–Jul 2026 (16,578 rows). Regenerated by `scripts/compute_levels.js`; cron fires 9:30 PM ET Sunday. |
...
| Auction/value | `developingValue.js`, `auctionRead.js`, `weekly.js`, `wyckoff.js`, `keyLevels.js` | POC/VAH/VAL tracking, opening-call classification (`open_vs_prior_value` + `overnight_inventory` auto-computed from price data; `prior_day_profile` is manual Sierra Chart read), weekly VA migration, Wyckoff levels, key-level regime stats. `keyLevels.js` also hosts `/api/level-prices/:date` (42 MGI levels) and `/api/level-prices/tag/:date` (re-tag BP fills). |
...
- `scripts/compute_levels.js` — computes all 42 MGI levels for a session date, writes to `level_prices`; supports single date or `--backfill [--from DATE]`; runs via cron 9:30 PM ET Sunday
