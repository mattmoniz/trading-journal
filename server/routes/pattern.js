...
// GET /api/behavioral-patterns — rows from performance_audit
// Optional: ?min_wr=70 (default 70), ?limit=200, ?signal_type=BEHAVIORAL_PATTERN
router.get('/behavioral-patterns', async (req, res) => {
  try {
    const minWr       = parseFloat(req.query.min_wr || '70') / 100;
    const limit       = parseInt(req.query.limit || '200');
    const signalType  = req.query.signal_type || 'BEHAVIORAL_PATTERN';
    const r = await query(`
