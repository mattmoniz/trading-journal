...
// MOVED 2026-09-01 to server/services/pocRotationService.js so acd.js's live poll
// loop can import the SAME audited detector (per CLAUDE.md's "export the real
// function" rule, applied in the direction of promoting a validated backtest
// mechanism to canonical-service status). Re-exported below unchanged so this
// script's own 14 downstream importers keep working without modification.
import { TICK, B_FLOOR, IncrementalProfile, subPoints, buildProfile, mean, percentile, detectSignalEvents as detectSignalEventsCore } from '../server/services/pocRotationService.js';
export { TICK, B_FLOOR, IncrementalProfile, subPoints, buildProfile, mean, percentile };
...
//
// Definition now lives in server/services/pocRotationService.js (moved 2026-09-01);
// re-exported here unchanged so every existing importer of this symbol keeps working.
export const detectSignalEvents = detectSignalEventsCore;
