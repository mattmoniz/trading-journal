...
| `prior_day_profile=TREND` anticipation flag (`auction_reads`, already computed) | A day preceded by a TREND day has ~1.8x the odds of itself being a 500+pt rotation day, AND real fade setups (`getBetClass()==='VALUE_FADE'` roster) net perform worse on TREND-preceded days | **Validated twice** — market-level: `RESEARCH_CLAIM` `prior_day_trend_profile_anticipates_rotation_day`, rechecked 2026-09-06 on fresh data (N=436, up from 408), χ²≈10.01, p≈0.0016, stronger than the original measurement. Real-setup-level: rechecked 2026-09-06 — the broad real fade roster (19 setup_types, N=1027) averages -$10.86/trade worse on TREND-preceded days (only 37% of types individually favorable), while 2 specific types (`IB_LOW_FADE_SHORT`, `PD_VAL_FADE_LONG`) buck the trend and do better | Zero-infrastructure-cost flag; **real risk signal for the broad fade roster, not a "size up" signal** — not yet wired into anything live; needs a mechanism decision (coaching flag / sizeMultiplier down-adjustment / dashboard badge) |
| Wide-IB range → LIVE-TURBULENT-reassessment hold-longer exit timing | Does holding a real trade longer (via the live `stepWiderTarget()` mechanism) pay off on a wide-IB-range day once the live day-type engine reassesses TURBULENT | **Real, confirms the theory, too thin to wire.** `RESEARCH_CLAIM` `wide_ib_turbulent_hold_longer_exit_timing_20260906` — holding longer costs -$15.09/trade on average (N=367 real trades), stable-signed across all 3 chronological thirds, but 100% of the sample sits in just 5 distinct dates | Not live. `OPEN_DECISION wire_intraday_ib_range_exit_signal` RESOLVED on this finding — no live wiring proposed, the answer is "don't hold longer here," which is already the status quo behavior |
| IB-boundary touch switch-count + directional-ratio (2D signature) | During the 60-min IB formation window, counts direction-alternations in "new extreme" events AND the up/down event-count ratio — distinguishes a clean lopsided push from lopsided-but-choppy grinding from genuine two-sided balance | **Real but thin, provisional.** `RESEARCH_CLAIM` `ib_switch_directional_ratio_2d_signature_20260906` — among wide-IB days, "Clean Leg" (low switch+lopsided, N=36) shows TREND=25.0%/TURBULENT=19.4% vs "Churn/Grind" (high switch+lopsided, N=48) showing TREND=16.7%/TURBULENT=31.3% — plain switch-count alone (tested first) found nothing; adding the directional-ratio dimension recovered this. No clean boost found yet for the fade roster; too thin (N=12 in the relevant bucket) to test against the newer `FAILED_SWEEP_REVERSAL` breakout roster | Not live, not yet validated (no placebo/parameter-robustness check run) |
| Day-Type Classifier v2 candidate — SCORED (2026-09-06) | Same 3 labels, IB-close reclassification | **Negative — do not deploy.** `RESEARCH_CLAIM` `daytype_v2_candidate_scored_negative_20260906`: 34.9% overall (N=421) vs v1's 45.1% — worse, not better; trades one error type (weak TREND recall) for a worse one (massively over-calls TURBULENT, 62% of all days) | Not live, verdict is final unless the spec itself is revised |
| Earlier-commit timing on Volatility Regime classifier — TESTED (2026-09-06) | Whether the validated 60-min-commit CHOP/DIRECTIONAL classifier holds up if committed at 15/30 min instead | **Negative — actively dangerous, not just weaker.** `RESEARCH_CLAIM` `vol_regime_early_commit_reverses_not_just_weakens_20260906`: the real (unit-corrected) 60-min split is -$48.66/tr DIRECTIONAL vs +$51.25/tr CHOP; at 30-min CHOP flips to -$15.68/tr; at 15-min DIRECTIONAL flips to +$13.46/tr — sign reversal, not just noise | Do not build an early-commit variant for live use |
| GARCH-forecast forward vol regime — attempted, BLOCKED (2026-09-06) | GARCH forecast vol as a pre-open regime label instead of realized vol | **Blocked on data coverage, not resolved either way.** Real, meaningfully different from realized-vol (35% agreement — not redundant), but `GARCH_VOL_SCALE` is a one-time backtest snapshot whose date coverage barely overlaps real `active_setups` history — too few real trades to test downstream EV | Not built; would need a GARCH re-run on recent dates or a trade-history backfill to actually test |
