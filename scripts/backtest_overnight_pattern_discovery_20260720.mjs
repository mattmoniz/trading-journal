...
function permTestProp(data, featureFn, outcomeFn, nPerm = 2000) {
  const realDiff = calcDiff(data, featureFn, outcomeFn);
  if (realDiff === null) return { p: 1, diff: 0, n1: 0, n2: 0 };
  ...
  return { p: hits / nPerm, diff: realDiff.diff, n1: realDiff.n1, n2: realDiff.n2, r1: realDiff.r1, r2: realDiff.r2 };
}
...
async function main() {
  ...
  // Split 70/30
  const splitIdx = Math.floor(moves.length * 0.7);
  const trainMoves = moves.slice(0, splitIdx);
  const testMoves = moves.slice(splitIdx);
  
  console.log(`Moves Train: ${trainMoves.length}, Test: ${testMoves.length}`);
  
  const results = [];
  
  // Test 1: Does startSession == '3_Asia' predict more DOWN moves vs other sessions?
  const testAsiaDown = {
    name: 'Asia Session -> DOWN Move',
    train: permTestProp(trainMoves, m => m.startSession === '3_Asia', m => m.moveType === 'DOWN' ? 1 : 0),
    test: permTestProp(testMoves, m => m.startSession === '3_Asia', m => m.moveType === 'DOWN' ? 1 : 0),
    rigor: computeRigor(testMoves.filter(m => m.startSession === '3_Asia'), { pnlFn: m => m.moveType === 'DOWN' ? 1 : -1 }),
    minN: testMoves.filter(m => m.startSession === '3_Asia').length >= 20 && testMoves.filter(m => m.startSession !== '3_Asia').length >= 20
  };
  results.push(testAsiaDown);

  // Test 2: Does prior RTH expansion predict a BIGGER overnight move?
  const testExpandedBigger = {
    name: 'Prior RTH Expanded -> Larger Move',
    train: permTestProp(trainMoves, m => m.isExpanded, m => m.totalDist),
    test: permTestProp(testMoves, m => m.isExpanded, m => m.totalDist),
    rigor: computeRigor(testMoves.filter(m => m.isExpanded), { pnlFn: m => m.totalDist > threshold * 1.5 ? 1 : -1 }),
    minN: testMoves.filter(m => m.isExpanded).length >= 20 && testMoves.filter(m => !m.isExpanded).length >= 20
  };
  results.push(testExpandedBigger);
  
  // Test 3: Does pre-inception compression (< median) predict larger moves?
  const medianComp = [...trainMoves].sort((a,b)=>a.preComp-b.preComp)[Math.floor(trainMoves.length/2)].preComp;
  const testCompBigger = {
    name: 'Pre-inception Coiled Spring -> Larger Move',
    train: permTestProp(trainMoves, m => m.preComp < medianComp, m => m.totalDist),
    test: permTestProp(testMoves, m => m.preComp < medianComp, m => m.totalDist),
    rigor: computeRigor(testMoves.filter(m => m.preComp < medianComp), { pnlFn: m => m.totalDist > threshold * 1.5 ? 1 : -1 }),
    minN: testMoves.filter(m => m.preComp < medianComp).length >= 20 && testMoves.filter(m => m.preComp >= medianComp).length >= 20
  };
  results.push(testCompBigger);

  // Test 4: Does a move with a 50% retrace predict a smaller final distance?
  const testRetraceSmaller = {
    name: 'Has 50% Retrace -> Smaller Final Dist',
    train: permTestProp(trainMoves, m => m.has50PctRetrace, m => -m.totalDist), // negative because we want "smaller"
    test: permTestProp(testMoves, m => m.has50PctRetrace, m => -m.totalDist),
    rigor: computeRigor(testMoves.filter(m => m.has50PctRetrace), { pnlFn: m => m.totalDist < threshold * 1.5 ? 1 : -1 }),
    minN: testMoves.filter(m => m.has50PctRetrace).length >= 20 && testMoves.filter(m => !m.has50PctRetrace).length >= 20
  };
  results.push(testRetraceSmaller);
  
  // Test 5: Does overnight direction predict RTH continuation?
  const testOvernightDirRth = {
    name: 'Overnight UP -> RTH Continues UP',
    train: permTestProp(trainMoves, m => m.moveType === 'UP', m => m.rthCont ? 1 : 0),
    test: permTestProp(testMoves, m => m.moveType === 'UP', m => m.rthCont ? 1 : 0),
    rigor: computeRigor(testMoves.filter(m => m.moveType === 'UP'), { pnlFn: m => m.rthCont ? 1 : -1 }),
    minN: testMoves.filter(m => m.moveType === 'UP').length >= 20 && testMoves.filter(m => m.moveType === 'DOWN').length >= 20
  };
  results.push(testOvernightDirRth);
  
  // Test 6: Is extending at 09:30 -> RTH Reversal?
  const testExtendingRev = {
    name: 'Still Extending at 09:30 -> RTH Reverses',
    train: permTestProp(trainMoves, m => m.isExtending, m => !m.rthCont ? 1 : 0),
    test: permTestProp(testMoves, m => m.isExtending, m => !m.rthCont ? 1 : 0),
    rigor: computeRigor(testMoves.filter(m => m.isExtending), { pnlFn: m => !m.rthCont ? 1 : -1 }),
    minN: testMoves.filter(m => m.isExtending).length >= 20 && testMoves.filter(m => !m.isExtending).length >= 20
  };
  results.push(testExtendingRev);

  // Test 7: Accel vs Decel predict continuation?
  const testAccelCont = {
    name: 'Accelerating Move -> RTH Continues',
    train: permTestProp(trainMoves, m => m.accelDecel === 'ACCEL', m => m.rthCont ? 1 : 0),
    test: permTestProp(testMoves, m => m.accelDecel === 'ACCEL', m => m.rthCont ? 1 : 0),
    rigor: computeRigor(testMoves.filter(m => m.accelDecel === 'ACCEL'), { pnlFn: m => m.rthCont ? 1 : -1 }),
    minN: testMoves.filter(m => m.accelDecel === 'ACCEL').length >= 20 && testMoves.filter(m => m.accelDecel === 'DECEL').length >= 20
  };
  results.push(testAccelCont);

  const survivors = [];
  let mdOut = `# Overnight Pattern Discovery Results\n\nThreshold: ${threshold.toFixed(2)} pts.\n`;
  
  for (const r of results) {
    const isSurvivor = r.test.p < 0.05 && r.minN && r.rigor.clean;
    if (isSurvivor) survivors.push(r);
    
    mdOut += `## ${r.name}\n`;
    mdOut += `- Train: Diff = ${r.train.diff.toFixed(3)} (p=${r.train.p.toFixed(3)})\n`;
    mdOut += `- Test: Diff = ${r.test.diff.toFixed(3)} (p=${r.test.p.toFixed(3)})\n`;
    mdOut += `- Min N met in test: ${r.minN} (n1=${r.test.n1}, n2=${r.test.n2})\n`;
    mdOut += `- Day clustering clean: ${r.rigor.clean}\n`;
    mdOut += `- Verdict: ${isSurvivor ? '**SURVIVED**' : 'Rejected'}\n\n`;
  }
  
  fs.writeFileSync('scratch/backtest_overnight_pattern_discovery_RESULTS.md', mdOut);
  ...
  await recordClaim({
    ...
    claimText: `Ran open-ended overnight pattern discovery across Precursors, Evolution, and RTH-Linkage. Found ${survivors.length} surviving patterns out of ${results.length} tested (requiring p<0.05 out-of-sample, N>=20, and clean day-clustering). ${survivors.length > 0 ? 'Survivors: ' + survivors.map(s => s.name).join(', ') : 'All candidates rejected by strict out-of-sample rigor.'}`,
    sourceFile: 'scripts/backtest_overnight_discovery.mjs',
    sourceDate: today,
    sampleSize: testMoves.length,
