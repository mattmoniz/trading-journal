...
export default function AlphaEngineOverview() {
  ...
  const suppressionMechanisms = [
    ...
    { name: 'OR5_LOW_FADE_LONG_GAP_DOWN [conditional variant, forced SHADOW]', reason: 'Fires when session 9:30 open is BELOW the prior session\'s RTH close (gap brought price down into the OR5 low, a support fade) — resolveSetupType() in acd.js converts the base OR5_LOW_FADE_LONG into this variant when the gap-down condition is met. Added 2026-08-18 from a mining pass over the OR family\'s bar-history population (N=147 aligned EV=$7.77/trade vs N=194 against EV=-$2.08, rigor CLEAN, replication PASS). Unlike WPP_FADE_SHORT_GAP_UP, this variant\'s dedicated script (scripts/backtest_or5_low_gap_down.mjs) hardcodes recommendation=THIN_N regardless of its own backtest EV — deliberately SHADOW-only until real forward trades reach N≥20, since WPP\'s let-the-threshold-decide pattern produced a setup rated ACTIVE with zero real origin_status=ACTIVE trades ever (an open INVARIANT_WARN, not a pattern to repeat). Promotion to ACTIVE is a deliberate manual step, not automatic.' },
