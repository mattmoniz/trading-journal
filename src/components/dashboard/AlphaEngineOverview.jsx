...
// Added 2026-07-17 after building the origin_status closed-loop fix -- a plain-language,
// visual map of the actual learning mechanism, since the mechanism itself (schema +
// scripts + a hook section) isn't visible anywhere in the app otherwise. No diagram
// library exists in this codebase; built with plain boxes + arrows to match this file's
// own inline-style convention rather than adding a new dependency for one diagram.
function LearningLoopStep({ title, desc, color, badge }) {
  return (
    <div style={{ ...S.card, borderLeft: `3px solid ${color}`, display: 'flex', flexDirection: 'column', gap: 4 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ fontSize: 13, fontWeight: 700, color: '#f1f5f9' }}>{title}</span>
        {badge && <span style={S.badge(color, color + '15')}>{badge}</span>}
      </div>
...
function LearningLoopArrow({ label }) {
  return (
    <div style={{ textAlign: 'center', padding: '2px 0' }}>
      ...
      {label && <div style={{ fontSize: 10, color: '#64748b', marginTop: 1 }}>{label}</div>}
    </div>
  );
}
...
function LearningLoopDiagram() {
  return (
    <div>
      ...
      <div style={{ textAlign: 'center', color: '#475569', fontSize: 11, marginTop: 6, marginBottom: 20 }}>
      ...
      <div style={S.grid2}>
        <div style={{ ...S.card, borderTop: '2px solid #a78bfa' }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: '#e2e8f0', marginBottom: 4 }}>Parallel loop — Pattern Discovery</div>
          <div style={{ fontSize: 12, color: '#94a3b8', lineHeight: 1.5 }}>Two scanners (rolling 90-day + all-time) continuously mine cross-cutting patterns and keep an ACTIVE/DEGRADED lifecycle — a pattern that stops working gets retired automatically, not left to look valid forever. Each pattern's win rate at discovery is now snapshotted permanently, so a future check can compare what it claimed against what it actually did.</div>
        </div>
        <div style={{ ...S.card, borderTop: '2px solid #64748b' }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: '#e2e8f0', marginBottom: 4 }}>Safety net — Data Sanity Audit</div>
          <div style={{ fontSize: 12, color: '#94a3b8', lineHeight: 1.5 }}>A weekly automated check for the kind of silent data-corruption that can poison every step above — impossible values, a wrong constant masquerading as a real one — so a future error gets caught by a standing check, not another manual deep-dive.</div>
        </div>
      </div>
    </div>
  );
