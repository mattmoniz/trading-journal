...
**`recommendation` field is now the single source of truth for "safe to use live"**: `VALIDATED_PAIR` requires `distinctDays>=20` (`PAIR_MIN_DISTINCT_DAYS`, reusing this codebase's existing N≥20 floor applied to the correct unit) AND `isCalibrated` AND `computeRigor()`'s `clean` flag AND positive EV, all in one place — the live consumer (`acd.js`'s `liveStats._pairBonus`) just filters on this one field rather than re-deriving the criteria. Corrected result: **71 pairs now qualify** (N 301-1291, EV $4-$20/trade) — up from 0 in the first uncalibrated 180-day pass. See `confluence_pair_proximity_2yr_backtest` `RESEARCH_CLAIM` for the full write-up.
