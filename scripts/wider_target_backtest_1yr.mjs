...
async function runBacktest(isRealOnly) {
  const originFilter = isRealOnly ? "('ACTIVE', 'SHADOW')" : "('ACTIVE', 'SHADOW', 'BACKFILL', 'UNKNOWN')";
  console.log(`Loading resolved trades with origin_status IN ${originFilter} over past 1 year...`);
  ...
  if (firedAtIds.length === 0) return { eligibleTrades: [], setupGroups: {}, summarizeGroup: () => ({}) };
  ...
  console.log('Loading NQ price bars over past 1 year...');
...
function getWinner(ds, N) {
  ...
  return { winnerValue, winnerStr, reason, confidence };
}

async function main() {
  const realRun = await runBacktest(true);
  const extRun = await runBacktest(false);

  let md = '# Wider Target Multiplier Backtest (1-Year Bound)\n\n';
  md += '**NOTE:** This is a SIMULATION/BACKTEST result, not realized P&L. Results span a 1-year history including synthetically-walked BACKFILL entries against historical price action.\n\n';

  const realTypeAnswers = {};
  for (const [st, trades] of Object.entries(realRun.setupGroups)) {
    const ds = realRun.summarizeGroup(trades);
    realTypeAnswers[st] = getWinner(ds, trades.length);
  }

  const typeResults = [];
  
  for (const [st, trades] of Object.entries(extRun.setupGroups)) {
    const ds = extRun.summarizeGroup(trades);
    const N = trades.length;
    
    const activeN = trades.filter(t => t.trade.origin_status === 'ACTIVE').length;
    const shadowN = trades.filter(t => t.trade.origin_status === 'SHADOW').length;
    const backfillN = trades.filter(t => t.trade.origin_status === 'BACKFILL').length;
    const unknownN = trades.filter(t => t.trade.origin_status === 'UNKNOWN').length;
    const originBreakdown = `${activeN} ACTIVE / ${shadowN} SHADOW / ${backfillN} BACKFILL / ${unknownN} UNKNOWN`;
    
    const fullWin = getWinner(ds, N);
    const realWin = realTypeAnswers[st];
    
    let agreement = 'N/A (Thin Real Data)';
    let flagConflict = false;
    if (realWin && realWin.confidence !== 'THIN') {
      if (realWin.winnerStr === fullWin.winnerStr) {
        agreement = 'AGREE';
      } else {
        agreement = `DISAGREE (Real suggests ${realWin.winnerStr})`;
        flagConflict = true;
      }
    }
    
    typeResults.push({
      st, N, originBreakdown, ds, flagConflict, agreement,
      ...fullWin
    });
  }
  ...
  md += '## Calibration Summary (Full 1-Year History)\n\n';
  md += '| Setup Type | Armed N | Origin Breakdown | Calibrated Mult | Confidence | Agrees w/ Real-Only? |\n';
  md += '|---|---|---|---|---|---|\n';
  for (const r of typeResults) {
    const flag = r.flagConflict ? '⚠️ ' : '';
    md += `| ${r.st} | ${r.N} | ${r.originBreakdown} | **${flag}${r.winnerStr}** | ${r.confidence} | ${r.agreement} |\n`;
  }
  
  md += '\n## Detailed Breakdown by Setup Type\n\n';
  for (const r of typeResults) {
    md += `### ${r.st} (N=${r.N})\n`;
    md += `*Origin composition: ${r.originBreakdown}*\n`;
    if (r.flagConflict) {
      md += `**⚠️ WARNING: Backtest winner (${r.winnerStr}) contradicts real-only run (${r.agreement})!**\n`;
    }
    md += '| Multiplier | EV Delta vs 1.0x | Rigor Verdict |\n';
    md += '|---|---|---|\n';
    for (const mult of WIDER_MULTIPLIERS) {
      if (mult === 1.0) {
        md += `| 1.0x | $0.00 | BASELINE |\n`;
        continue;
      }
      const s = r.ds[mult];
      ...
      md += `| ${mult}x | $${s.meanDelta.toFixed(2)} | ${rigorVerdict} |\n`;
    }
    md += '\n';
  }

  fs.writeFileSync('scratch/wider_target_full_backtest_1yr_RESULTS.md', md);
  console.log('Results written to scratch/wider_target_full_backtest_1yr_RESULTS.md');
}
