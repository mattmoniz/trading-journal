...
## 2026-08-18: duplicate server supervisors caused a real RTH detection gap — single-instance takeover consolidated

User noticed zero live `ACTIVE` fires all day (40+ setups fired, all `SHADOW`) against a 4-13/day baseline every prior day this week. Traced to 5 simultaneous `nodemon server/index.js` processes fighting over port 3002: one surviving from a post-WSL-reboot `restart.sh` session (~1:58 AM, plus a full separate `npm start` tree from the same event still serving a 12-hour-stale frontend on port 3000), and three more from separate sessions independently running bare `npm run server` across the day with zero cleanup between them. The resulting `EADDRINUSE` crash-restart storm concentrated 11:30 AM–1:09 PM ET — prime RTH — eventually tripped `trading-journal-server.service`'s 2026-08-09 failure-burst override into a permanent `failed` state (systemd does not auto-recover from `failed`), leaving production served only by an unsupervised ad hoc process.

Root cause was **duplicated, drifted safety logic**, not a one-off mistake: `restart.sh` had hand-copied `start.sh`'s `cleanup_ports()` months ago and silently drifted out of sync — missing the systemd-stop-first step and the EXIT-trap/health-monitor handback `start.sh` has (this exact drift was flagged as "worth a look next session" in `docs/OPEN_THREADS.md`'s 2026-08-17 entry and sat unresolved until it caused a real outage). Bare `npm run server` — CLAUDE.md's own documented "just the backend" shortcut — had *none* of this protection at all, so every session that used it stacked a permanent extra supervisor.

**Fix**: extracted the shared logic into `scripts/lib/server-lifecycle.sh`, sourced by `start.sh`, the now-thin-wrapper `restart.sh`, `stop.sh`, and the new `scripts/dev-server-guard.sh` (what `npm run server` now runs, via `package.json`). A `SERVER_GUARD_PARENT_MANAGED` env check keeps the guard from double-running its own cleanup/trap/health-monitor when invoked as `start.sh`'s own nested child through `concurrently`. Added a backstop to `.claude/hooks/session-start.sh` that loudly flags >1 simultaneous `nodemon server/index.js` supervisor in case anything still bypasses all three entry points. This is the same "share modules instead of reimplementing" lesson CLAUDE.md's Conventions section already names for other subsystems — the fix here generalizes it to the dev-server lifecycle scripts themselves, which had never been through that discipline before.

**Verified**: all 5 stale/duplicate processes killed live, systemd's failure state reset (left `inactive`, not restarted, so as not to disrupt the one healthy process already serving mid-trading-day), all 4 shell scripts pass `bash -n`, `session-start.sh` runs end-to-end clean with the new duplicate-check correctly silent against the single remaining supervisor.
