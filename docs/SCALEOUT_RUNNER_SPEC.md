...
Status: **§1-§9 BUILT AND LIVE (2026-07-19, SHADOW-only) for `FLOOR_R1_FADE_SHORT_TRAIL`**, and **all remaining 5 survivors wired the same way 2026-07-21** (`PW_HIGH_FADE_LONG_TRAIL`, `PD_POC_FADE_LONG_TRAIL`, `FLOOR_S1_FADE_LONG_TRAIL`, `DAILY_OPEN_FADE_LONG_TRAIL`, `CAM_S2_FADE_LONG_TRAIL` — `CONDITIONAL_VARIANTS` entries in `server/config/setupTypes.js`, `resolveSetupType()` overrides in `acd.js`, `SETUP_DISPLAY_LABELS` entries, `test_invariants.mjs` calibration-coverage check all passing for all 6). Originally written 2026-07-19 as a fractional scale-out spec; **fully revised the same day** after the user confirmed a 1-contract execution constraint invalidated the fractional design, and a corrected backtest (2 Gemini rounds, both audited) landed on a different, simpler mechanism with different survivors. This is the current, durable version — the fractional-split design (partial-exit columns, two-leg P&L blending, `_SCALEOUT` variant naming) is fully superseded and should not be built. Full build writeup: docs/OPEN_THREADS.md's 2026-07-19 and 2026-07-21 entries.

**Important caveat found 2026-07-21, before wiring the other 5**: `FLOOR_R1_FADE_SHORT_TRAIL` had **zero real (non-suppressed) live touches** in the ~2 days since being wired — its own documented gating condition ("add the other 5 once this one proves itself at live N≥20") was not actually met before the other 5 were wired. Not a bug (the one row that exists is a correctly-minimal `SUPPRESSED_FADE`/`S2_DOUBLE_COUNTER` audit entry, same shape as every other suppressed-touch row in this table) — just a fact worth knowing: **none of the 6 has real forward validation yet, only the backtest.** Wired anyway per explicit user direction, SHADOW-only so no live capital is at risk either way. Remaining: the §7 display polish (deferred — SHADOW-only produces no live card right now), and the §8 live-health recheck script (needs live data to exist first, for all 6 now, not just 1).
...
`resolution_method` gets new values: `SAME_BAR_ARM_STOP`, `BREAKEVEN_TRAIL_HIT`, or `TRAIL_TIME_EXPIRED` if still open at session close — §6 (kept ≤20 chars, `resolution_method` is `VARCHAR(20)`; the original longer names `SAME_BAR_ARM_AND_STOP`/`BREAKEVEN_TIME_EXPIRED` would have thrown a DB error and crashed `resolveSetupsByPrice()` for every setup on first real use — caught 2026-07-20 before it ever fired). `actual_pnl` is just the normal single-leg formula against wherever the (possibly-ratcheted) stop or target closed the trade — no blending, no fraction math. This is a genuinely simpler build than the original fractional design: no `'PARTIAL'` status needed (the row stays `ACTIVE`/`SHADOW` the whole time, just with a dynamically-moving stop), no two-leg P&L formula, no new setup_type variant naming scheme (see §4).
