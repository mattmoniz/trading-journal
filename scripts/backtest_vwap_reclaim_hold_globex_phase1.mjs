...
// Load 1-minute bars for NQ Globex
async function loadBars() {
  const r = await query(`
    SELECT ts::text as ts, 
      (CASE 
        WHEN EXTRACT(hour FROM ts) >= 18 THEN ts::date + interval '1 day'
        ELSE ts::date 
      END)::date::text as trade_date,
    ...
    WHERE symbol = 'NQ' 
      AND (EXTRACT(hour FROM ts) >= 18 OR (EXTRACT(hour FROM ts)*60 + EXTRACT(minute FROM ts)) < 510)
    ORDER BY ts
  `);
  return r.rows;
}
...
function build5MinBars(bars1m) {
  ...
  for (const b of bars1m) {
    const minutesFromStart = b.mod >= 1080 ? b.mod - 1080 : b.mod + 360;
    const intervalIdx = Math.floor(minutesFromStart / 5);
    
    if (!current5m || current5m.intervalIdx !== intervalIdx) {
      if (current5m) bars5m.push(current5m);
      current5m = {
        ts: b.ts, trade_date: b.trade_date, intervalIdx,
...
async function run() {
  ...
  console.log('Loading 1-min Globex bars...');
  const bars1m = await loadBars();
  console.log(`Loaded ${bars1m.length} 1-min Globex bars.`);
  ...
  for (const K of K_VALUES) {
    for (const dir of ['LONG', 'SHORT']) {
      ...
      if (candTrades.length >= 20) {
        await recordClaim({
          slug: `vwap_reclaim_hold_globex_k${K}_${dir.toLowerCase()}_phase1`,
          claimText: `VWAP reclaim-and-hold GLOBEX phase 1. K=${K} ${dir}. Target swept on MFE (max p75) with structural VWAP cross-back stop.`,
          sourceFile: 'scripts/backtest_vwap_reclaim_hold_globex_phase1.mjs',
  ...
  let mdOut = '# VWAP Reclaim and Hold GLOBEX Backtest Results\n\n';
