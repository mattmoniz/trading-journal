...
async function getLiveStatus(setupType) {
  ...
  // promotion this same session): this is the REAL live gate for RTH_FLUSH_LONG/SHORT (per this
  // file's own design, decoupled from backtest_flush_patterns.mjs's informational SETUP_STATUS
  // row), so it needs the same origin_status IN ('ACTIVE','SHADOW') scoping every other real-N
  // gate in this codebase uses -- without it, any future BACKFILL/UNKNOWN-origin reconstruction
  // for this setup_type would silently count toward live eligibility. Currently a latent gap,
  // not an active bug: real n for both flush types is 1-3 today, all SHADOW-origin already.
  const { rows } = await query(`
