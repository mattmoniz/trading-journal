  ...
  return (
    <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: 8, padding: '12px 16px', marginBottom: 12 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer' }} onClick={() => setCollapsed(!collapsed)}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 9, color: '#475569' }}>{ts}</span>
        ...
        <div style={{ marginTop: 8, fontSize: 12, lineHeight: 1.7, color: '#cbd5e1' }}>
          ...
          {data.topDowCombos?.length > 0 && (
            <div style={{ marginBottom: 8 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#4ade80', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 3 }}>
                ...
                return (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '2px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    ...
                    <span style={{ fontSize: 11 }}>
                      <span style={{ color: p.wr >= 75 ? '#4ade80' : '#fbbf24', fontWeight: 700 }}>{p.wr}%</span> · N={p.n} · ${p.pnl}
                    </span>
                  </div>
                );
              })}
          ...
          {data.bestHours?.length > 0 && (
            <div style={{ marginBottom: 8 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#818cf8', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 3 }}>
          ...
          {data.pipelineSetups?.length > 0 && (
            <div style={{ marginBottom: 8 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#f472b6', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 3 }}>
              ...
              {data.pipelineSetups.map((s, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '2px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                  ...
                  <span style={{ fontSize: 11, color: '#94a3b8' }}>
                    {s.wr != null ? <><span style={{ color: s.wr >= 60 ? '#4ade80' : s.wr >= 45 ? '#fbbf24' : '#f87171', fontWeight: 700 }}>{s.wr}%</span> (N={s.n})</> : `${s.fires} fires`}
                    {s.timeWindow && <> · <span style={{ color: '#64748b' }}>{s.timeWindow}</span></>}
                  </span>
                </div>
              ))}
            </div>
          ...
          {data.coilWatch?.length > 0 && (
            <div style={{ marginBottom: 8 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#f97316', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 3 }}>
              ...
              {data.coilWatch.map((c, i) => (
                <div key={i} style={{ padding: '2px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                  ...
                  {c.coiled && c.coilRatio && (
                    <div style={{ fontSize: 10, color: '#f97316' }}>COILED — next win avg ${c.avgDroughtWin} ({c.coilRatio}x normal). Hold for runner.</div>
                  )}
                </div>
              ))}
            </div>
          ...
          {data.overnightProfile && (
            <div style={{ marginBottom: 8 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#06b6d4', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 3 }}>
                Overnight Structure
              </div>
          ...
          {data.balanceZones?.length > 0 && (
            <div style={{ marginBottom: 8 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#8b5cf6', textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 3 }}>
          ...
          {sc && sc.scalps.trades + sc.vwapMagnet.trades + sc.pipeline.trades > 0 && (
            <div style={{ padding: '8px', background: 'rgba(255,255,255,0.03)', borderRadius: 4, border: '1px solid rgba(255,255,255,0.06)' }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#4ade80', textTransform: 'uppercase', marginBottom: 4 }}>Today's Results</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 6 }}>
                {[
                  ...
                  <div key={i}>
                    <div style={{ fontSize: 9, color: '#64748b' }}>{cat.label}</div>
